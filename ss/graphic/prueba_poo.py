...
class Application:
    def __init__(self, root):
        ...
        # Create buttons
        self.create_button(
            text="Show", command=self.show_window,
            row=9, column=6, columnspan=3, rowspan=1,
            bg="green", fg="white"
        )
        self.create_button(
            text="Hide", command=self.hide_window,
            row=9, column=11, columnspan=3, rowspan=1,
            bg="red", fg="white"
        )

        # Create text entries
        self.create_entry(
            row=5, column=5, columnspan=5, rowspan=1, show_text=True
        )
        self.create_entry(
            row=7, column=5, columnspan=5, rowspan=1, show_text=False
        )

        # Create a label example
        self.create_label(
            text="Welcome to the system",
            row=3, column=5, columnspan=10, rowspan=2,
            bg="white", fg="blue", font=("Arial", 20, "bold")
        )
    ...
    def add_background(self):
        image = Image.open("resources/f_sc_5.jpg")
